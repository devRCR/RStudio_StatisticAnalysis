...
modelo=glm(diabetes~.,data=PimaIndiansDiabetes2,
           family=binomial())
...
choose.files()
setwd("C:\\Users\\ING-J\\Downloads\\SESION9\\")
getwd()
library(readxl)
# ----------------  arbol  decision   ------------------
library(rpart)
...
particion<-createDataPartition(datos$DEATH_EVENT,p=0.75,list=FALSE)
...
particion<-createDataPartition(iris$Species,
                    p=0.8,list=FALSE)
...
#ver el error como va disminuyend por cantidad arboles
plot(modelo)
