...
setwd("C:\\Users\\ING-J\\Downloads\\SESION8\\")
getwd()
...
#ver de forma grafico modelo lineal,poly a 2, poly a 3
library(ggplot2)
ggplot(train,
       aes(x=CONCENTRACION,y=RESISTENCIA))+
         geom_point()+#mostrar data real
         geom_smooth(method="lm",formula=y~x,se=FALSE,
                     col="brown")+ #modelo lineal
         geom_smooth(method="lm",formula=y~poly(x,2),se=FALSE,
                     col="green")+ #modelo polinomial al cuadrado
         geom_smooth(method="lm",formula=y~poly(x,3),se=FALSE,
                     col="red")+ #modelo polinomial al cubo
         theme_light()
